...
export const CoachInviteMessage = ({ user }: UserProps) => {
  ...
  return (
    <>
      {user.clientStatus && user.clientStatus && user.clientStatus.users ? (
        <div
          className={`${user.clientStatus && user.clientStatus.status === "Pending" ? "" : "hidden"} ${!open ? "hidden" : ""} fixed inset-0 z-[100] flex items-center justify-center overscroll-none bg-black/75 backdrop-blur-sm`}
        >
          <div
